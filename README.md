...
> 将两种长度的歌的数量进行遍历，如果满足总长度为K，计算该数量的组合<a href="https://www.codecogs.com/eqnedit.php?latex=C_{n}^{m}" target="_blank"><img src="https://latex.codecogs.com/svg.latex?C_{n}^{m}" title="C_{n}^{m}" /></a>下共有多少种不同歌曲的组合方式，所以本题的关键是实现组合函数：
